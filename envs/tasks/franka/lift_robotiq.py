...
from ...robots import FrankaWithRobotiqGripper
...
_CONFIG_FILE_NAME = 'franka/lift_robotiq.json'

def franka_lift_robotiq(time_limit=_DEFAULT_TIME_LIMIT, environment_kwargs=None):
    """Create a franka env, aiming to lift a cube.
    """
    config_path = os.path.join(_SUITE_DIR, './configs', _CONFIG_FILE_NAME)
    with open(config_path, mode='r') as f:
        config = json.load(f)
    robot = FrankaWithRobotiqGripper.from_file_path(
        xml_path=config['xml'],
        asset_paths=config['assets'],
        actuator_path=os.path.join(_FRANKA_XML_DIR, 'actuator_robotiq.xml'),
        mocap_path=os.path.join(_FRANKA_XML_DIR, 'mocap_robotiq.xml'),
        config=config
    )
