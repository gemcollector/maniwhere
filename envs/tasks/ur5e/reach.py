...
_CONFIG_FILE_NAME = 'ur5e/reach.json'

def ur5_reach(time_limit=_DEFAULT_TIME_LIMIT, random=None, environment_kwargs=None):
...
class Physics(RandPhysics):
    def end_to_target(self):
        ...
        end_to_target = data.site_xpos['target'] + np.array([0, 0, 0.02]) - \
            (data.site_xpos['right_fingertip'] + data.site_xpos['left_fingertip']) / 2
...
class Reach(BaseTask):
    ...
    def __init__(
        self,
        target_low=(0.6, -0.15, 0.001),
        target_high=(0.8, 0.15, 0.001),
        random=None,
        action_delay=0
    ):
    ...
    def initialize_episode(self, physics):
        ...
        """
        physics.named.model.site_pos['target'] = np.random.uniform(
